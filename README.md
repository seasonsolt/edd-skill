...
EDD 是 Eval-Driven Development。这个 repo 做的是一件很小但很硬的事：把 TDD 的 red-green-refactor 循环迁移到 AI coding agent 上。Agent 不再直接开写，而是先把任务变成测试、eval 或 benchmark，再围绕这些可执行标准迭代。
...
传统 TDD 约束的是人写代码的顺序。EDD Skill 约束的是 agent loop 的顺序：
...
这轮结果很克制：EDD Skill 没有提升功能正确性，因为 baseline agent 也把功能做对了。它提升的是开发过程质量，with-skill run 留下了 `AI_TDD_REPORT.md`、`evals/red.log`、`evals/green.log`，后续可以复盘 red state、green state 和新增回归。
...
如果 with-skill 只提高过程分，说明 EDD 主要改善可审计性。如果它还提高 hidden pass rate，才说明它开始影响最终质量。
